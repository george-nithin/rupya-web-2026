        ...
        <GlassCard className="overflow-hidden">
            <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex gap-2">
                    <span className="px-3 py-1 bg-card/20 rounded-full text-xs text-muted-foreground border border-border/50">Nifty 50</span>
                    <span className="px-3 py-1 bg-card/20 rounded-full text-xs text-muted-foreground border border-border/50">Sector Leaders</span>
                </div>
            </div>
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="text-xs text-muted-foreground uppercase tracking-wider border-b border-border/50 bg-white/[0.02]">
                    ...
                    <tbody className="text-sm">
                        {peers.map((peer, index) => {
                            ...
                            return (
                                <tr
                                    key={peer.symbol}
                                    className={`border-b border-white/5 last:border-0 hover:bg-white/5 transition-all duration-150 ${isCurrent ? "bg-sky-500/10" : ""}`}
                                >
                                    <td className="px-4 py-4">
                                        <div className="flex items-center gap-3">
                                            <div className="h-8 w-8 rounded bg-card/30 flex items-center justify-center text-[10px] font-bold text-foreground/80">
                                            ...
                                            <div>
                                                ...
                                                <div className="text-xs text-muted-foreground">{peer.sector}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-4 py-4 text-right text-foreground font-mono">
                                    ...
                                    <td className="px-4 py-4 text-right text-muted-foreground font-mono">{peer.marketCap}</td>
                                    <td className="px-4 py-4 text-right">
                                        <div className="inline-block px-2 py-1 rounded bg-card/20 text-foreground/80 border border-border/50 text-xs font-bold">
                                            {peer.pe}x
                                        </div>
                                    </td>
                                    <td className="px-4 py-4 text-right text-foreground/80 font-mono">{peer.roe}%</td>
                                    <td className="px-4 py-4 text-center">
                                        <div className="h-1.5 w-16 bg-card/30 rounded-full overflow-hidden mx-auto">
                                            <div
            ...
            <div className="mt-4 flex justify-end">
                <button className="text-xs text-sky-400 hover:text-sky-300 flex items-center gap-1 transition-all duration-150">
