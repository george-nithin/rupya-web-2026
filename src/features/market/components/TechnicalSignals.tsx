...
export function TechnicalSignals({ data }: TechnicalSignalsProps) {
    ...
    const getSignal = (timeframe: string) => {
        ...
        if (timeframe === "1D") {
            ...
            return { type: "NEUTRAL", label: "Neutral", color: "text-muted-foreground", bg: "bg-slate-500/10", border: "border-slate-500/20" };
        }
        ...
        const sentiments = [
            ...
            { type: "NEUTRAL", label: "Neutral", color: "text-muted-foreground", bg: "bg-slate-500/10", border: "border-slate-500/20" },
            ...
            <GlassCard>
                ...
                <div className="space-y-4">
                    {intradayTimeframes.map((tf) => {
                        ...
                        return (
                            <div key={tf} className="flex items-center justify-between p-3 rounded-xl bg-card/20 border border-border/50">
                                <span className="text-xs font-bold text-muted-foreground w-16">{tf}</span>
                ...
                <div className="mt-4 pt-4 border-t border-border/50 text-[10px] text-muted-foreground text-center">
            ...
            <GlassCard>
                ...
                <div className="space-y-4">
                    {longTermTimeframes.map((item) => {
                        ...
                        return (
                            <div key={item.tf} className="p-3 rounded-xl bg-card/20 border border-border/50">
                                <div className="flex justify-between items-center mb-2">
                                    <span className="text-xs font-bold text-foreground/80">{item.tf}</span>
