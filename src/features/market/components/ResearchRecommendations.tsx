        ...
        <div className="space-y-4">
            <div className="flex items-center justify-between">
                ...
                <button className="text-xs text-sky-400 hover:text-white flex items-center gap-1">
                    VIEW ALL TRADING IDEAS <ArrowRight className="h-4 w-4" />
                </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {loading ? (
                    [1, 2, 3].map(i => (
                        <div key={i} className="h-64 bg-slate-900/40 rounded-xl border border-white/5 animate-pulse" />
                    ))
                ) : recs.length > 0 ? (
                    recs.map((rec) => (
                        <div key={rec.id} className="rounded-xl overflow-hidden border border-white/10 bg-[#0f1115] hover:border-sky-500/30 transition-all group relative">
                            ...
                            <div className="p-4 border-b border-white/5">
                                <h3 className="font-bold text-white text-md tracking-wide">{rec.symbol}</h3>
                                <div className="flex items-center gap-2 mt-1">
                                    <span className="text-[10px] text-slate-500">24 Feb 2026</span>
                                    <span className="text-[10px] text-purple-400 bg-purple-500/10 px-1 rounded border border-purple-500/20">CE</span>
                                </div>
                                <div className="mt-2">
                                    <span className="text-[9px] text-slate-400 uppercase border border-white/10 px-1.5 py-0.5 rounded bg-white/5 tracking-wider">
                                        {rec.timeframe}
                                    </span>
                                </div>
                            </div>
                            ...
                            <div className="p-4 bg-[#0a0c10]/50">
                                <div className="flex justify-between items-end mb-4">
                                    ...
                                    <div className="text-right">
                                        <div className="text-[10px] text-slate-500 mb-0.5">Recommended Price:</div>
                                ...
                                <div className="flex justify-between items-center pt-3 border-t border-dashed border-white/10">
                                    <div>
                                        ...
                                        <div className="text-sm font-bold text-white font-mono">{rec.target_price}</div>
                                    </div>
                                    <div className="text-right">
                                        <div className="text-[10px] text-slate-500 mb-0.5">StopLoss:</div>
                                        <div className="text-sm font-bold text-slate-300 font-mono">{rec.stop_loss}</div>
                                    </div>
                                </div>
                            </div>

                            {/* Card Footer Actions */}
                            <div className="p-3 bg-[#0a0c10] border-t border-white/5 flex justify-between items-center">
                                <div className="text-[9px] text-slate-600">
                                    Updated: {new Date(rec.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                </div>

                                <div className="flex items-center gap-2">
                                    <button className="p-1.5 text-slate-500 hover:text-white bg-white/5 rounded transition-colors border border-white/5 hover:border-white/20">
                                    ...
                                    <button className="p-1.5 text-slate-500 hover:text-yellow-400 bg-white/5 rounded transition-colors border border-white/5 hover:border-white/20">
