    ...
    return (
        <div className="max-w-4xl mx-auto">
            <div className="flex items-center gap-4 mb-6">
                ...
                <h1 className="text-2xl font-bold text-white">Design Strategy</h1>
            </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <p className="text-xs text-slate-500 mt-1">This will be used for auto-position sizing.</p>
                            </div>

                            <div>
                                <label className="block text-sm font-medium text-slate-400 mb-2">Preferred Timeframes</label>
                                <div className="flex flex-wrap gap-2">
                                    {['1m', '5m', '15m', '1H', '4H', '1D'].map(tf => (
                                        <label key={tf} className="flex items-center gap-2 bg-white/5 px-3 py-1.5 rounded cursor-pointer hover:bg-white/10">
                                            <input type="checkbox" value={tf} {...register("timeframes")} className="rounded bg-slate-800 border-white/10 text-sky-500" />
                                            <span className="text-sm text-slate-300">{tf}</span>
                                        </label>
                                    ))}
                                </div>
                ...
                <GlassCard className="p-6">
                    ...
                    <div className="space-y-2">
                        {rules.length === 0 && (
                            <div className="text-center py-8 text-slate-500 italic">No rules defined yet.</div>
                        )}
                        {rules.map((rule) => (
                            <div key={rule.id} className="flex items-center justify-between p-3 bg-white/5 rounded-lg border border-white/5 group">
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className="text-slate-200 text-sm">{rule.text}</span>
                                </div>
