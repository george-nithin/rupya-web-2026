    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-2xl font-bold text-white">Strategy Playbook</h1>
                    <p className="text-slate-400">Manage and refine your trading systems</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {strategies.map((strategy) => (
                    <GlassCard key={strategy.id} className="group relative overflow-hidden transition-all hover:border-sky-500/30">
                        <div className="absolute top-0 right-0 p-4 opacity-0 group-hover:opacity-100 transition-opacity flex gap-2">
                            <button className="p-1 hover:bg-white/10 rounded text-slate-400 hover:text-white" title="Duplicate">
                                <Copy className="h-4 w-4" />
                            </button>
                            <button className="p-1 hover:bg-white/10 rounded text-slate-400 hover:text-white" title="Archive">
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-6">
                            ...
                            <div>
                                ...
                                <div className="text-xl font-bold text-white">₹{strategy.total_pnl.toLocaleString()}</div>
                            </div>
                        </div>
                ...
                <Link href="/dashboard/strategies/create" className="block">
                    <div className="h-full min-h-[200px] border-2 border-dashed border-white/10 rounded-2xl flex flex-col items-center justify-center text-slate-500 hover:border-sky-500/50 hover:bg-sky-500/5 transition-all cursor-pointer group">
                        ...
                        <span className="font-medium group-hover:text-white">Design New System</span>
                    </div>
                </Link>
            </div>
        </div>
