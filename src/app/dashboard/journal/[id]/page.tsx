            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex gap-3">
                    <Link href={`/dashboard/journal/edit/${id}`}>
                        <GlassButton>
                            <Pencil className="h-5 w-5 mr-2" /> Edit Trade
                        </GlassButton>
                    </Link>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <div className="lg:col-span-8 space-y-6">
                    <GlassCard className="min-h-[500px] flex flex-col p-0 overflow-hidden relative group bg-card border-border/50">
                        <div className="p-4 border-b border-border/50 flex justify-between items-center bg-card/20">
                            ...
                            <div className="flex gap-2">
                                <span className="text-xs px-2 py-1 bg-card/20 rounded text-muted-foreground">1H</span>
                                <span className="text-xs px-2 py-1 bg-card/20 rounded text-muted-foreground">1D</span>
                            </div>
                        </div>
                                ...
                                <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <a href={trade.screenshot_url} target="_blank" rel="noopener noreferrer">
                                        <GlassButton size="sm" variant="secondary">
                                            <ExternalLink className="h-5 w-5 mr-2" /> Full Screen
                                        </GlassButton>
                                    </a>
                                </div>
                            </div>
                        ...
                        <div className="p-4 bg-card/20 border-t border-border/50">
                            <div className="text-[10px] text-muted-foreground uppercase mb-1 font-bold tracking-wider">Analysis Notes</div>
                    ...
                    <div className="space-y-3">
                        <h3 className="text-sm font-bold text-muted-foreground uppercase tracking-widest pl-1">Recent History: {trade.symbol}</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {relatedTrades.map((t) => (
                                <Link key={t.id} href={`/dashboard/journal/${t.id}`}>
                                    <GlassCard className="p-3 hover:bg-card/20 transition-all duration-150 cursor-pointer flex items-center justify-between group active:scale-95">
                                        <div className="flex items-center gap-3">
                                            <div className={`w-1 h-8 rounded-full ${t.status === 'OPEN' ? 'bg-sky-500' : (t.pnl || 0) >= 0 ? 'bg-green-500' : 'bg-red-500'}`} />
                                            <div>
                                                <div className="text-xs text-muted-foreground">{new Date(t.entry_date).toLocaleDateString()}</div>
                ...
                <div className="lg:col-span-4 space-y-6">
                    ...
                    <GlassCard className="bg-[#111316] border-border p-5 space-y-6">
                        <div className="flex justify-between items-start">
                            ...
                            <div className={`px-3 py-1 rounded-xl text-xs font-bold border ${trade.status === 'OPEN' ? 'bg-sky-500/10 text-sky-400 border-sky-500/20' : isProfit ? 'bg-green-500/10 text-green-400 border-green-500/20' : 'bg-red-500/10 text-red-400 border-red-500/20'}`}>
                                {trade.status}
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 pt-4 border-t border-border/50">
                            <div>
                                ...
                                <span className="text-lg font-mono text-foreground">₹{trade.entry_price}</span>
                            </div>
                            <div>
                                <span className="text-xs text-muted-foreground block">Exit</span>
                                <span className="text-lg font-mono text-foreground">{trade.exit_price || '-'}</span>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <span className="text-lg font-mono text-foreground">{trade.quantity}</span>
                            </div>
                        </div>
                        ...
                        <div className="space-y-3 pt-4 border-t border-border/50">
                            <div className="flex justify-between items-center text-sm">
                                ...
                                <span className="text-foreground font-medium">{trade.strategy_name || "N/A"}</span>
                            </div>
                            <div className="flex justify-between items-center text-sm">
                                <span className="text-muted-foreground">Session</span>
                                <span className="text-foreground font-medium">{trade.session || "N/A"}</span>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-2 pt-2">
                            ...
                            {trade.tags?.length > 0 && trade.tags.map((t: string) => (
                                <span key={t} className="text-[10px] px-2 py-1 bg-secondary text-foreground/80 rounded border border-border uppercase font-bold">
                                    #{t}
                                </span>
                            ))}
                        </div>
                    ...
                    <Link href={`/dashboard/market/${trade.symbol}`}>
                        <GlassButton className="w-full" variant="secondary">
                            View Stock Details <ChevronRight className="h-5 w-5 ml-auto" />
                        </GlassButton>
                    </Link>
                </div>
            </div>
