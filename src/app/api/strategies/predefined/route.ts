...
class TradingStrategy:
    ...
    def initialize(self, portfolio: Portfolio, data: pd.DataFrame):
        """Initialize strategy with portfolio and data. Do not override this usually."""
    ...
    def on_data(self, timestamp: datetime, current_bar: Dict, historical_data: pd.DataFrame) -> List[Order]:
        ...
        REQUIRED: Must ensure this method returns a list of orders (empty list if no action).
        
        Args:
            timestamp: Current timestamp (datetime)
            current_bar: Dict with 'open', 'high', 'low', 'close', 'volume'
            historical_data: DataFrame with all data up to current timestamp
            
        Returns:
            List[Order]: List of Order objects to execute
        """
    ...
    def calculate_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        """Calculate trading signals (Optional helper method)"""
        return data
`
};
...
export async function GET(request: NextRequest) {
    ...
    if (id && STRATEGY_CODES[id]) {
        return NextResponse.json({ code: STRATEGY_CODES[id] });
    }

    return NextResponse.json({
        strategies: Object.keys(STRATEGY_CODES).filter(k => k !== 'base_structure'),
        base_structure: STRATEGY_CODES['base_structure']
    });
}
